...
public class WebGame {
	...
	public static void main(String[] args) {
		...
		//tic.playGame();
		String html = "<div style='float:right; width:200px; font-weight:bold;' id='message_prompt'>X has move</div><button onclick='makeMove(1)' id='1'>1</button>  <button onclick='makeMove(2)' id='2'>2</button>  <button onclick='makeMove(3)' id='3'>3</button> <br/><button onclick='makeMove(4)' id='4'>4</button>  <button onclick='makeMove(5)' id='5'>5</button>  <button onclick='makeMove(6)' id='6'>6</button> <br/><button onclick='makeMove(7)' id='7'>7</button>  <button onclick='makeMove(8)' id='8'>8</button>  <button onclick='makeMove(9)' id='9'>9</button> <br/> <button onclick='newGame();' id='new_game'>New game</button> <script src='https://code.jquery.com/jquery-3.2.1.min.js' integrity='sha256-hwg4gsxgFZhOsEEamdOYGBf13FyQuiTwlAQgxVSNgt4=' crossorigin='anonymous'></script><script>function makeMove(field) {   $.get('/makeMove/'+field, { 'field': field }, function(data) { var current_player = '';  if(data.length == 1) { $('#'+field).html(data); current_player = 'x'; /*data.toString();*/  /*isTie();*/ } else {/*if(data.indexOf('x') != -1) { $('#'+field).html('x'); current_player = 'x'; } else if(data.indexOf('o') != -1) { $('#'+field).html('o'); current_player = 'o' } alert(data);*/ } /*var next_player; if(current_player == 'x') { next_player = 'o'; } else { next_player = 'x'; } alert(current_player); alert(next_player);*/ /*$('#message_prompt').html(next_player.toUpperCase()+' has move');*/ });  } function newGame() { $.get('/newGame/', function(data) { $('#message_prompt').html('X has move'); var counter = 1; $('button').each(function() { if($(this).attr('id') != 'new_game') { $(this).html(counter); counter++; } });  }); }  function isTie() { var tie = true; $('button').each(function() { if($(this).html() != 'x' && $(this).html() != 'o' && $(this).attr('id') != 'new_game') { tie = false; } }); if(tie) { alert('Tie game'); } }</script>";
		get("/", (req, res) -> html);
